    ...
    <>
      ...
      <nav className="w-full fixed top-0 left-0 bg-white text-black shadow-md px-6 py-4 flex items-center z-50">
          ...
          {navLinks.map(({ path, label }) => (
            <Link
              ...
              className={`transition font-medium ${
                location.pathname === path ? 'text-[#FD1F4A]' : ''
              }`}
            >
              {label}
            </Link>
        ...
        <div className="hidden md:block ml-auto">
          <Link
            to="/quotepage"
            className="bg-[#FD1F4A] text-white font-semibold px-4 py-2 rounded-lg transition"
          >
            Get Quote
          </Link>
        </div>
      ...
      >
        ...
        <div className="px-6 mt-4">
          <Link
            to="/quotepage"
            className="block w-full text-center bg-[#FD1F4A] text-white font-semibold px-4 py-2 rounded-lg transition"
            onClick={() => setMenuOpen(false)}
          >
            Get Quote
          </Link>
